...
class InvoiceSystem:
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute('''CREATE TABLE IF NOT EXISTS invoice_tags
                         (invoice_id INTEGER,
                         tag_id INTEGER,
    ...
    def process_invoice(self, image_path, uploader, tags=[]):
        ...
        try:
            # Step 1: 调用OCR识别
            invoice_data = self._ocr_invoice(image_path)
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Step 3: 检查重复
                cursor.execute('SELECT id FROM invoices WHERE unique_code=?', 
                ...
                # Step 7: 更新总金额
                cursor.execute('''UPDATE invoices SET total_amount = 
    ...
    def _process_tags(self, cursor, invoice_id, tags):
        ...
        for tag_name in tags:
            ...
            cursor.execute('INSERT OR IGNORE INTO invoice_tags VALUES (?,?)', 
                         (invoice_id, tag_id))

    def _ocr_invoice(self, image_path):
        """模拟OCR处理（需替换为实际API调用）"""
        # 示例数据模板
        sample_data = {
            ...
            'total_amount': 568.0,
            'items': [
                {'product_name': '办公椅', 'quantity': 2, 'unit_price': 200, 'amount': 400},
                {'product_name': '打印纸', 'quantity': 10, 'unit_price': 16.8, 'amount': 168}
            ]
        }
        return sample_data
...
if __name__ == "__main__":
    ...
    for idx, case in enumerate(test_cases):
        result = system.process_invoice(case['image'], case['uploader'], case.get('tags', []))
    ...
    # 打印统计结果
    print(f"处理完成：新增 {stats['added']} 张，重复 {stats['duplicates']} 张，错误 {len(stats['errors'])} 条")
